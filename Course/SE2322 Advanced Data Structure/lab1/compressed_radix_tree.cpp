...
void CompressedRadixTree::insert(int32_t value)
{
    ...
    for (int32_t i = 30; i >= 0;)
    {
        ...
        if (!current->children[index])
        {
            ...
            std::cout << "inserted address " << current->children[index] << std::endl;
            std::cout << "inserted value " << current->children[index]->value << std::endl;
            std::cout << "inserted length " << current->children[index]->length << std::endl;
            _size++;
            print();
            return;
        }
        ...
        if (value_cmp_part != child_value)
        {
            ...
            while (common_prefix < current_child_length && ((value_cmp_part >> (current_child_length - common_prefix - 1)) & 1) == ((child_value >> (current_child_length - common_prefix - 1)) & 1))
                common_prefix++;
            std::cout << "common_prefix of " << value_cmp_part << " and " << child_value << " is " << common_prefix << std::endl;
            ...
            std::cout << "new_node address " << new_node << std::endl;
            std::cout << "new_node value " << new_node->value << std::endl;
            std::cout << "new_node length " << new_node->length << std::endl;
            ...
            std::cout << "new_node2_index " << new_node2_index << std::endl;
            std::cout << "new_node2 inserted address " << new_node2 << std::endl;
            std::cout << "new_node2 inserted value " << new_node2->value << std::endl;
            std::cout << "new_node2 inserted length " << new_node2->length << std::endl;
            ...
            std::cout << "new_node3_index " << new_node3_index << std::endl;
            std::cout << "new_node3 inserted address " << new_node3 << std::endl;
            std::cout << "new_node3 inserted value " << new_node3->value << std::endl;
            std::cout << "new_node3 inserted length " << new_node3->length << std::endl;
            // 用tmp释放原节点的空间
...
bool CompressedRadixTree::find(int32_t value)
{
    ...
    for (int32_t i = 30; i >= 0;)
    {
        ...
        if (current_child_length == 32) // 特判32位的情况，防止溢出
            ...
            value_cmp_part = (value >> (i + 2 - current_child_length)) & ((1 << current_child_length) - 1);
        std::cout << "value_cmp_part " << value_cmp_part << std::endl;
        std::cout << "current->children[index]->value " << current->children[index]->value << std::endl;
        // 如果不相等，返回false
        if (value_cmp_part != current->children[index]->value)
            return false;
        // 移动到下一个节点
