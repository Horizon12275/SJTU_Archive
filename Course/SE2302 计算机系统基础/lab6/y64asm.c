 ...
 *     NULL: not exist
 */
symbol_t *find_symbol(char *name) // find symbol
{
 ...
 */
int add_symbol(char *name) // add symbol
{
    /* check duplicate */
    if(find_symbol(name))
        return -1;
 ...
 *     name: the name of symbol
 */
void add_reloc(char *name, bin_t *bin) // relocation add
{
 ...
 */
parse_t parse_instr(char **ptr, instr_t **inst) // parse instruction
{
    /* skip the blank */
    SKIP_BLANK(*ptr);
 ...
 */
parse_t parse_reg(char **ptr, regid_t *regid) // parse register
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
    if (!IS_REG(*ptr))
 ...
 */
parse_t parse_symbol(char **ptr, char **name) // parse symbol
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
    if (!IS_LETTER(*ptr))
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    char *p;
    *value = strtoll(*ptr, &p, 0);

    /* handle the situtation that digit exceeds INT_MAX, manually convert the value*/
 ...
 */
parse_t parse_imm(char **ptr, char **name, long *value) // parse immediate
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
 ...
 */
parse_t parse_mem(char **ptr, long *value, regid_t *regid) // parse memory
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
 ...
 */
parse_t parse_label(char **ptr, char **name) // parse label
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
    if (!IS_LETTER(*ptr))
...
*           call SUM  #invoke SUM function */
    ...
    if (parse_instr(&pt_ins, &inst) == PARSE_INSTR)
    {
        ...
        case I_RET: 
            break; // do nothing
        case I_RRMOVQ:
        case I_ALU: // rrmovq and alu
            if (parse_reg(&pt_ins, &regA) == PARSE_ERR) // parse register
            {
                err_print("Invalid REG");
                return TYPE_ERR;
            }
            if (parse_delim(&pt_ins, ',') == PARSE_ERR) // parse delimiter
            {
                err_print("Invalid ','");
                return TYPE_ERR;
            }
            if (parse_reg(&pt_ins, &regB) == PARSE_ERR) // parse register
            {
                err_print("Invalid REG");
                return TYPE_ERR;
            }
            line->y64bin.codes[1] = HPACK(regA, regB); // set the register
            break;
        case I_RMMOVQ:
            if (parse_reg(&pt_ins, &regA) == PARSE_ERR) // parse register
            {
            ...
            if (parse_mem(&pt_ins, &value, &regB) == PARSE_ERR) // parse memory
            {
                err_print("Invalid MEM");
                return TYPE_ERR;
            }
            line->y64bin.codes[1] = HPACK(regA, regB); // set the register
            for (int i = 0; i < 8; i++) // set the value
                line->y64bin.codes[i + 2] = (value >> (8 * i)) & 0xFF; 
            break;
        case I_IRMOVQ:
            type = parse_imm(&pt_ins, &name, &value); // parse immediate
            if (type == PARSE_ERR)
            {
            ...
            if (type == PARSE_DIGIT)
                ...
                add_reloc(name, &line->y64bin); // add relocation
            break;
        case I_MRMOVQ:
            if (parse_mem(&pt_ins, &value, &regB) == PARSE_ERR) // parse memory
            {
        ...
        case I_POPQ:
            ...
            line->y64bin.codes[1] = HPACK(regA, REG_NONE); // set the register
            break;
        case I_JMP:
        case I_CALL:
            type = parse_imm(&pt_ins, &name, &value); // parse immediate
            ...
            if (type == PARSE_DIGIT)
                ...
                add_reloc(name, &line->y64bin); // add relocation
            break;
        case I_DIRECTIVE: // handle the situation like .align, .pos, .byte, .word, .long, .quad
            if (d_code == D_DATA) // .byte, .word, .long, .quad
            {
...
int relocate(void)
{
    ...
    while (r)
    {
        ...
        itype_t type = HIGH(r->y64bin->codes[0]);

        if (type == I_IRMOVQ)
            for (int i = 0; i < 8; ++i)
                r->y64bin->codes[i + 2] = (symbol->addr >> (8 * i)) & 0xFF;
        else if (type == I_JMP || type == I_CALL)
            for (int i = 0; i < 8; ++i)
                r->y64bin->codes[i + 1] = (symbol->addr >> (8 * i)) & 0xFF;
        else if (type == I_DIRECTIVE)
            for (int i = 0; i < r->y64bin->bytes; ++i)
                r->y64bin->codes[i] = (symbol->addr >> (8 * i)) & 0xFF;
        /* next */
        r = r->next;
    }
    return 0;
 ...
 *     -1: error
 */
int binfile(FILE *out) // generate binary file
{
    ...
    while (tmp != NULL) {
        if (tmp->type == TYPE_INS) {
            fseek(out, tmp->y64bin.addr, SEEK_SET); // move to the position
